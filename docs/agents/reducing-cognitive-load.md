# Reducing developer cognitive load with agents
...
> Profile: `wcag-22-aa`. Surface: the login form in `app/views/sessions/new.html.erb`. Use `docs/profiles/requirement-profiles.md` and filter `docs/wcag/success-criteria.md`.

Profiles: [../profiles/requirement-profiles.md](../profiles/requirement-profiles.md).
...
| --- | --- |
| Choosing the bar | `docs/profiles/requirement-profiles.md` only |
| Implementing a surface | Catalog rows for that surface + those Understanding URLs |
| Writing a conformance note | `docs/wcag/conformance.md` + profile card |
| Building an authoring tool / generator | `docs/authoring-tools/atag.md` + 508 §504 |
| After AA passes, improving usability | `docs/cognitive/coga-usable.md` (advisory) |
...
W3C already designed this layering: Quick Reference → Understanding → Techniques. The agent should follow it. Local: [../wcag/documents.md](../wcag/documents.md).
...
- Do not claim the **page** conforms because one component looks fine ([conformance is per full page and complete process](../wcag/conformance.md)).
...
That is ATAG 2.0 Part B applied to a coding agent. Local: [../authoring-tools/atag.md](../authoring-tools/atag.md).
...
Local: [../evaluation/wcag-em.md](../evaluation/wcag-em.md).

## 8. Prompt patterns that stay small

**Implement**

> You are implementing under profile `ada-title-ii`. Read `docs/profiles/requirement-profiles.md` and only the **ADA-II = Y** rows in `docs/wcag/success-criteria.md` that apply to forms. Change only the password-reset flow. For each change, cite the criterion id and Understanding URL.
