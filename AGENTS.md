...
2. Prefer **Markdown** under `source/docs/` for the official-docs library (`docs` layout). Knowledge-tree nodes live under **`source/learn/`** (`learn` layout). The graph is **`data/tree.yml`** (ids, prereqs, children, spine, library links). `kind: pattern` is a family; `kind: rule` is one cop (frontmatter `rule:`, Bad and Good on the same page). `kind: example` is a live specimen under a rule (`source/learn/<rule>/bad.html.erb` and `good.html.erb`, `example` layout). Cops that need the whole document (`<title>`, `lang` on `<html>`, a live keyboard trap) add `window: true` on the host example and `source/learn/<rule>/bad/window.html.erb` (`window` layout). The host links **Open example in a new window**. Close that tab to leave. Do not put a live trap on the host page. Same site header as the rest of the site; no extra headings in `<main>`, so a heading list still sees only that markup. The `specimen` Stimulus controller adds Edit. Pick, fix, and isolated examples show approximate **Screen Reader** after Rendered (outside `<main>`; not a real screen reader). Compact widths use Error / HTML / Rendered / Screen Reader tabs. The editor is created on demand from the live HTML in `<main>` and removed from the DOM when closed, so tools do not see a textarea unless you are editing. `kind: practice` is a locator or constructed item (`pick.html.erb` / `fix.html.erb`, `practice` layout) with an in-house check. Checkers take a DOM node and score only that node (and its descendants), never the rest of the page. Pick: mark every card that follows the rule. A correct pick collapses; a miss shows the rule report on that card (no modal). When every card is classified, the page says so. Fix **Apply** writes into `<main>` and scores that node. Failures list rules with links. A family hub can have combined pick/fix (`check_rules` lists the child cops). Add a node when you add a page. Use ERB for chrome and interactive demos. Publishable pages are for the **learner**. Host names, “not installed here,” and other lab notes go in **`notes/`** (not built). Do not copy a rule’s specimen walkthrough onto every tool page; link the rule instead.
